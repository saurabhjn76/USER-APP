...
public class Maps_toolbar_activity extends AppCompatActivity implements OnMapReadyCallback {

    private GoogleMap mMap;
    private ArrayList<String> address= new ArrayList<>(50);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(getIntent().hasExtra("size"))
        {
            ...
            {
                if(getIntent().hasExtra("Salon_address"+i))
                {
                    Toast.makeText(getApplicationContext(),bundle.getString("Salon_address"+i),Toast.LENGTH_SHORT).show();
                    address.add(i,bundle.getString("Salon_address"+i));
                }
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        for(int i=0;i<size;i++) {
            gurgaon = getLocationFromAddress(getApplicationContext(), address.get(i));
//            Toast.makeText(getApplicationContext(), gurgaon.toString(), Toast.LENGTH_LONG).show();
            try {
                mMap.addMarker(new MarkerOptions().position(gurgaon).title(""+i));
            }
            catch (Exception e)
            {
            ...
            mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(gurgaon, 10.0f));
        }
